...
export function createGoblin(scene) {
  const g = scene.physics.add.sprite(200,200,'goblin_run')
  ...
  g.takeDamage = function(dmg=1){
    ...
    this.once(
      Phaser.Animations.Events.ANIMATION_COMPLETE_KEY+'goblin_hurt',
      ()=>{
        if(this.hp<=0){
          scene.tweens.add({
            targets: this,
...
function createAnimations(scene){
  ...
  dirs.forEach((dir,i)=>{
    scene.anims.create({
      key:`goblin_run_${dir}`,
      frames: scene.anims.generateFrameNumbers('goblin_run',{start:i*8,end:i*8+7}),
...
export function updateGoblin(scene,g,player){
  ...
  const dx=player.x-g.x, dy=player.y-g.y, distSq=dx*dx+dy*dy;
  const dir = Math.abs(dx)>Math.abs(dy)?(dx<0?'left':'right'):(dy<0?'up':'down');

  if(distSq>g.detectRadiusSq){
    if(g.state!=='idle'){
      g.state='idle'; g.setVelocity(0); g.play('goblin_idle',true);
    }
  } else if(distSq>g.attackRadiusSq){
    if(g.state!=='chase') g.state='chase';
    g.play(`goblin_run_${dir}`,true);
    scene.physics.moveToObject(g,player,g.speed);
  } else if(scene.time.now - g.lastAttackTime >= g.attackCooldown){
    g.state='attack'; g.setVelocity(0);
    g.attackSound.play();
    g.play(`goblin_attack_${dir}`,true);
    g.once(
      Phaser.Animations.Events.ANIMATION_COMPLETE_KEY+`goblin_attack_${dir}`,
      ()=>{
