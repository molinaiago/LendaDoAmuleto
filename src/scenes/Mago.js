...
export function createMago(scene){
  const m = scene.physics.add.sprite(0,0,'mago_walk')
    .setSize(32,48).setOffset(16,16)
    .setImmovable(true).setCollideWorldBounds(true);
  ...
  m.takeDamage = function(dmg=1){
    ...
    this.once(
      Phaser.Animations.Events.ANIMATION_COMPLETE_KEY+'mago_hurt',
      ()=>{
        if(this.hp<=0){
          // fade & destroy
          scene.tweens.add({
...
export function updateMago(scene,m,player){
  ...
  const dx=player.x-m.x, dy=player.y-m.y, distSq=dx*dx+dy*dy;
  const dir = Math.abs(dx)>Math.abs(dy)?(dx<0?'left':'right'):(dy<0?'up':'down');

  if(distSq>m.detectRadiusSq){
    if(m.state!=='idle'){ m.state='idle'; m.play('mago_walk_down',true); m.setVelocity(0);}
  } else if(distSq>m.attackRadiusSq){
    if(m.state!=='chase') m.state='chase';
    m.play(`mago_walk_${dir}`,true);
    scene.physics.moveToObject(m,player,m.speed);
  } else if(scene.time.now - m.lastAttackTime >= m.attackCooldown){
    m.state='attack'; m.setVelocity(0);
    m.attackSound.play();
    m.play(`mago_attack_${dir}`,true);
    m.once(
      Phaser.Animations.Events.ANIMATION_COMPLETE_KEY+`mago_attack_${dir}`,
      ()=>{
